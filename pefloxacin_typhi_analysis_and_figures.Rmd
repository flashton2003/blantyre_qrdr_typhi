...
Data has been pulled in from various sources and cleaned using `pefloxacin_typhi_data_cleaning.Rmd`. The clean data has been saved as `r cases_bc_qrdr_prescrip_microconf_handle`.
...
The number of sequencing confirmed typhoid cases and number of qrdr (sequencing confirmed), number of prescriptions of cipro, and number of blood cultures.
...
micro_conf_n <- sum(entire_recruitment_period$micro_confirmed_n)
```


In total, over the entire study period there were:

* `r micro_conf_n` cases of microbiologically confirmed typhoid.
...
broom.mixed::augment(m, entire_recruitment_period, scale="response")
exp(-1.2036)
library(sjPlot)

plot_model(m, type="pred")

# predict the qrdr for each month with glm.pred
# convert the output to probability scale
# want to make a predicted model because we have the "real world" data, but that's all confounded by the various other factors.
# the model corrects for those confounders, and makes a prediction of the effect of the cipro prescriptions only on the number of qrdr isolated each month.
