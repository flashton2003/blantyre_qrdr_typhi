...
Question - should we be using `total_seq_typhoid_n-total_qrdr_n` or just `total_seq_typhoid_n`?

```{r glm model}

m <- glm(cbind(total_qrdr_n, total_seq_typhoid_n-total_qrdr_n) ~ total_prescrip_n + total_bc_n,  
...
entire_recruitment_period <- entire_recruitment_period %>% mutate(total_non_qrdr_typhi = total_seq_typhoid_n-total_qrdr_n)

x <- cbind(entire_recruitment_period$total_qrdr_n, entire_recruitment_period$total_seq_typhoid_n-entire_recruitment_period$total_qrdr_n)

m1 <- glm(cbind(total_qrdr_n, total_seq_typhoid_n-total_qrdr_n) ~ lag(total_prescrip_n) + total_bc_n,  
         data = entire_recruitment_period, 
         family = "binomial")
...
preds %>%
  ...
  geom_point(aes(x=Month, y=total_qrdr_n/total_seq_typhoid_n), colour="firebrick", alpha=0.5, shape=4) +
...
m2 <- brm(bf(total_qrdr_n | trials(total_seq_typhoid_n) ~ total_prescrip_n + total_bc_n),
          data = entire_recruitment_period, 
          family = binomial(),
          chains = 4)
...
preds2 %>%
  mutate(.epred_prev = .epred/total_seq_typhoid_n,
         .lower_prev = .lower/total_seq_typhoid_n,
         .upper_prev = .upper/total_seq_typhoid_n) %>%
...
# todo - need to get access to the qrdr mutations per month here for colouring in figure 2.
# can get from 2022.09.15.qrdr_typhi.one_per_patient.csv
