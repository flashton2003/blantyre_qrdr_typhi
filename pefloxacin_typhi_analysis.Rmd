...
all_typhi <- all_typhi %>% mutate(any_qrdr = ifelse(is.na(QRDR), NA, "QRDR"))
...
todo:
* make sure excluding multiple isolates from the same person
...
two ways of getting the qrdr:
1. group by qrdr, and the pivot wider (or something)
2. filter by qrdr, make new count tables, do some joins
