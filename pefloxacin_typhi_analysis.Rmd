...
# these dates are the start and finish of the period of overlap between strataa and tyvac
...
case_and_prescription_incidence <- data.frame(Month = seq(tyvac_strataa_start, tyvac_strataa_end, "months")) 
cases_incidence_per_month <- select(combined_cases_by_month, c('Month', 'total_incidence_per100000'))
prescriptions_incidence_per_month <- select(combined_prescrip_by_month, c('Month', 'total_prescrip_per_100000'))

case_and_prescription_incidence <- left_join(case_and_prescription_incidence, cases_incidence_per_month, by = 'Month')
case_and_prescription_incidence <- left_join(case_and_prescription_incidence, prescriptions_incidence_per_month, by = 'Month')
...
We will initially construct a zero-inflated negative Bbinomial regression model to investigate this association. We select the zero-inflated negative binomial distributional family because it is likely that the data are overdispersed, and because of the excess zero months.

```{r}

m1 <- glmmTMB(any_qrdr ~ cipro,  data = case_and_prescription_incidence, family = "nbinom1")
