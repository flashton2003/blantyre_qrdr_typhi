...
run_models <- function(input_data) {
  m1 <- glmmTMB(total_qrdr_incidence_per_100000 ~ total_prescrip_per_100000,  data = input_data, family = "nbinom1")
  #summary(m1)
  m1_summary <- broom.mixed::tidy(m1, exponentiate = TRUE, conf.int = TRUE)
  
  m2 <- glmmTMB(total_qrdr_incidence_per_100000 ~ total_prescrip_per_100000 + total_bc_per_100000,  data = input_data, family = "nbinom1")
  #summary(m2)
  m2_summary <- broom.mixed::tidy(m2, exponentiate = TRUE, conf.int = TRUE)
  ...
  model_summaries <- list(m1_summary = m1_summary, m2_summary = m2_summary, m3_summary = m3_summary)
...
model_summaries$m1_summary %>% kbl() %>% kable_styling()
model_summaries$m2_summary %>% kbl() %>% kable_styling()
model_summaries$m3_summary %>% kbl() %>% kable_styling()
...
```{r Figure2.v2, fig.width = 6, fig.height = 12}


typhoid_graph <- ggplot(strataa_tyvac_recruitment_period, aes(x = Month, y = total_typhoid_per100000)) + 
  geom_bar(stat = "identity") + 
  ylab('Sequencing confirmed typhoid\nper 100,000 people')

cipro_prescrip_graph <- ggplot(strataa_tyvac_recruitment_period, aes(x = Month, y = total_prescrip_per_100000)) + 
  geom_bar(stat = "identity") + 
  ylab('Ciprofloxacin prescriptions\nper 100,000 people')

qrdr_graph <- ggplot(strataa_tyvac_recruitment_period, aes(x = Month, y = total_qrdr_incidence_per_100000)) + 
  geom_bar(stat = "identity") +
  ylab('S. Typhi with QRDR mutations\nper 100,000 people')

#qrdr_graph

bc_graph <- ggplot(strataa_tyvac_recruitment_period, aes(x = Month, y = total_bc_per_100000)) + 
  geom_bar(stat = "identity") +
  ylab('Number of blood cultures\nper 100,000 people')

#typhoid_graph
#cipro_prescrip_graph
#qrdr_graph
#bc_graph
p <- typhoid_graph / cipro_prescrip_graph / qrdr_graph / bc_graph + plot_layout(heights = c(5,5,5,5))
